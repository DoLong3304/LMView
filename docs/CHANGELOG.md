...
- **Test count** — 341 → 382 pytest tests (added 41 metrics tests).

### Added (Phase 5 dashboard completion)
...
### Added (Phase 5 Flink writer instrumentation)
...
- **Test count** — 341 → 363 pytest tests (added 22 writer metrics tests).

### Added (Phase 5 Bottleneck Mitigations)
...
- **`backend/api/metrics.py`** — Thêm section "Trino query pool" với 4 metric + 2 helper (`record_trino_query`, `record_trino_fallback`). Update `__all__`.
- **`backend/api/market_overview.py`** — `AsyncTrinoClient.fetch_one/fetch_all` thêm `query_type` parameter. Track active queries + record duration/failure/fallback. 8 helper functions (`_get_market_summary`, `_get_top_movers`, ...) giờ pass `query_type` cụ thể.

### Added (Phase 5 Real Fixes)
...
- **A9.1 (Frontend RUM)**:
  ...
  - **`config/grafana/provisioning/alerting/rules.yml`** — group mới `frontend_rum_alerts` (folder `LMView / Frontend`) với alert `A9.1: Frontend JS error rate elevated` (rate > 0.5/s trong 5m → warning).
  - **`backend/app.py`** — register `rum` router.
...
- **A10.2 (API rate limit hit)**:
  ...
  - **`backend/app.py`** — register middleware (sau CORS).
  - **`config/grafana/provisioning/alerting/rules.yml`** — group mới `api_security_alerts` (folder `LMView / Security`) với 2 alerts: `A10.1: Auth failure spike` (rate > 2/s trong 5m → warning) và `A10.2: API rate limit hit` (rate > 1/s trong 5m → info).
...
- **Phase 5 tests:** 97 → 124 (added 9 AI wire-up + 6 rate-limit + 9 indicator-state + 2 integration existing = 26 new tests).
- **Total unit tests:** 377 → 401 (excluding 4 sentiment tests với LLM mock issue không liên quan).

...
function logRumWarning(message: string, error: unknown): void {
...
export function installRum(): void {
  ...
  try {
    ...
    logRumWarning("RUM: LCP observer unavailable", e);
  }
  ...
  try {
    ...
    logRumWarning("RUM: INP observer unavailable", e);
  }

  // Periodic flush
  window.setInterval(flush, BATCH_INTERVAL_MS);
...
export async function flush(): Promise<void> {
  ...
  try {
    ...
    logRumWarning("RUM: flush failed", e);
  }
}

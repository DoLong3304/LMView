import type { Candle, Ticker, Trade, NewsItem } from "../types";
import { TIMEFRAMES } from "../services/marketDataService";
...
export function generateMockMarketOverview() {
  const t = Date.now();
  return {
...
export function generateMockGainers() {
  return [
    { symbol: "PEPEUSDT", change_24h_pct: +(15.4 + Math.random() * 2).toFixed(2) },
    { symbol: "SOLUSDT", change_24h_pct: +(8.2 + Math.random() * 1).toFixed(2) },
    { symbol: "DOGEUSDT", change_24h_pct: +(6.5 + Math.random() * 1).toFixed(2) },
    { symbol: "AVAXUSDT", change_24h_pct: +(5.1 + Math.random() * 1).toFixed(2) },
    { symbol: "LINKUSDT", change_24h_pct: +(4.8 + Math.random() * 1).toFixed(2) },
  ];
}

export function generateMockLosers() {
  return [
    { symbol: "ADAUSDT", change_24h_pct: +(-6.2 - Math.random() * 2).toFixed(2) },
    { symbol: "XRPUSDT", change_24h_pct: +(-4.5 - Math.random() * 1).toFixed(2) },
    { symbol: "DOTUSDT", change_24h_pct: +(-3.8 - Math.random() * 1).toFixed(2) },
    { symbol: "MATICUSDT", change_24h_pct: +(-2.9 - Math.random() * 1).toFixed(2) },
    { symbol: "LTCUSDT", change_24h_pct: +(-2.1 - Math.random() * 1).toFixed(2) },
  ];
}

...
import { fetchOrderBook, type RawOrderBookData } from "@/services/marketDataService";
    ...
    const load = () => {
      fetchOrderBook(symbol)
        .then((data) => {
          ...
          const parseSide = (arr: RawOrderBookData["bids"]): OrderBookEntry[] => {
            let running = 0;
            return (arr || []).map(([p, q]) => {
          ...
          setBook({ asks, bids, spread: Number(data.spread || 0) });
          setError(null);
        })
        .catch(() => {
